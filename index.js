...
const shouldCluster = isProduction && process.env.DISABLE_CLUSTER !== "true";

// Helper function that creates and configures the Express app
function createApp() {
  const app = express();
  ...
  app.get("/", (req, res) => {
    res.json({
      ...
      worker: cluster.isWorker ? `Worker ${process.pid}` : "Primary",
    });
  });
...
if (cluster.isPrimary && shouldCluster) {
  ...
  // Worker (or non-clustered mode) starts the real server
